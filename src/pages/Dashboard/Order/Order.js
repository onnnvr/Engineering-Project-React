...
export default function Order() {
    ...
    if (!order && !loading) return <div className="p-10 text-center">{t('Order not found')}</div>;
        ...
        <div className="bg-gray-50/50 min-h-screen pb-20 text-left">
            ...
            <div className="relative bg-white border-b px-4 md:px-6 py-4 flex flex-wrap gap-4 justify-between items-center shadow-sm top-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg md:text-xl font-bold text-gray-800">{t('Order Details')}</h2>
                ...
                <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                    ...
                    <span className={`flex-1 md:flex-none text-center px-3 py-2 rounded-xl text-xs font-bold border ${getStatusStyles(order?.orderStatus)}`}>
                        {order?.orderStatus ? t(order?.orderStatus) : t('Delivered')}
                    </span>

                    {!order?.paid && order?.orderStatus !== 'Returned' && (
                        <button 
                ...
                <div className="md:col-span-8 space-y-6">
                    ...
                    <div className="bg-white p-4 md:p-8 rounded-3xl border border-gray-100 shadow-sm">
                        <div className="flex items-center gap-3 mb-6 text-gray-800">
                            ...
                            <h3 className="text-lg font-bold">{t('Items')} ({order?.order_items?.length})</h3>
                        </div>
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-6 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100">
                            <div className="relative pl-10">
                                ...
                                <p className="font-bold text-gray-800 text-sm md:text-base">{t('Order Placed')}</p>
                                <p className="text-[10px] md:text-xs text-gray-400">{formatDate(order?.createdAt)}</p>
                            </div>
                            ...
                            {order?.orderStatus === 'Returned' && (
                                <div className="relative pl-10">
                                    ...
                                    <p className="font-bold text-red-600 text-sm">{t('Order Returned')}</p>
                ...
                <div className="md:col-span-4 space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-1">
                        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <h4 className="font-bold text-gray-800 text-sm truncate">{order?.user?.username}</h4>
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                            <div className="flex items-center gap-3 text-blue-600 font-bold mb-4">
                                ...
                                <h3 className="uppercase tracking-widest text-[10px]">{t('Customer')}</h3>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center font-bold">
                                    {order?.customer?.name?.charAt(0).toUpperCase()}
                                </div>
                                <h4 className="font-bold text-gray-800 text-sm truncate">{order?.customer?.name}</h4>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm sticky bottom-6 md:relative md:bottom-0">
                        ...
                        <div className="space-y-4 border-b border-gray-50 pb-6 text-sm">
                            ...
                            <div className="flex justify-between items-center text-gray-500">
                                <span>{t('Taxes (14%)')}</span>
                                <span className="text-gray-800 font-bold">${order?.taxAmount}</span>
                            </div>
                            {!order?.paid && (
